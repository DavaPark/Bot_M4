...
class InvoiceStatusResult:
	def __init__(self, response_dict, reason, reasonCode, orderReference, amount, currency, authCode, createdDate,
	             processingDate, cardPan, cardType, issuerBankCountry, issuerBankName, transactionStatus, refundAmount,
	             settlementDate, settlementAmount, fee, merchantSignature):
...
class WayForPay:
	...
	def create_invoice(self, merchantAccount, merchantAuthType, amount, currency, orderID, *args, **kwargs):
		...
		params = {
			...
			"language": "UA",
			"serviceUrl": "https://m4ukraine.com.ua/api/paymentCallback/"
		}
	...
	def check_invoice(self, merchantAccount, orderReference):
		...
		try:
			...
			if result.status_code == 200:
				...
				return InvoiceStatusResult(response_dict, reason, reasonCode, orderReference, amount, currency,
				                           authCode, createdDate, processingDate, cardPan, cardType, issuerBankCountry,
				                           issuerBankName, transactionStatus, refundAmount, settlementDate,
				                           settlementAmount, fee, merchantSignature)
